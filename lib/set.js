...
Set.prototype._migrate = function(direction, migrationName, fn){
  ...
  function next(common, migration) {
    ...
    migration[direction](function(err){
      ...
      self.save(function (err) {
        ...
        next(common, migrations.shift())
      });
    }, common);
  }

  if (isGenerator(this.common))
    co(this.common)
      .catch(fn)
      .then(function(common) { next(common, migrations.shift()); });
  else
    next(this.common, migrations.shift());
};
